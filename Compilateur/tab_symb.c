...
int ts_create (char *id, int init, int constant, int nest) { // ajout d'un élément dans la table
	...
	(tab->ts[tab->sp]).constant=constant;
	(tab->ts[tab->sp]).nesting=nest;
	(tab->sp)++;
	// printf("sp = %d\n",(tab->sp)-1);
	return ((tab->sp)-1);
}
...
void ts_print (){
	int i;
	for (i=0;i<(tab->sp);i++){
		printf("Variable à adresse %d: %s ; nestlevel = %d\n",i,tab->ts[i].name, tab->ts[i].nesting);
		}
	// printf("test\n");
}
